...
uint32_t float_to_hex(float f) { return *((uint32_t *)&f); }
...
void Load() { *(CGRA_info.reg_mmap + LOAD_BASE_PHYS) = 1; }

void Start() { *(CGRA_info.reg_mmap + START_BASE_PHYS) = 1; }

void Done() { *(CGRA_info.reg_mmap + DONE_BASE_PHYS) = 1; }
    ...
    for (int loop = run_from; loop <= run_from + step; loop++) {
        for (int num_qubits = 3; num_qubits < 18; num_qubits++) {
            for (int depth = 1; depth < 11; depth++) {
                ...
                char dir_path[100];
                snprintf(dir_path, sizeof(dir_path), "Output/%d/", loop);
                ...
                // Create the filename
                char filename[100];
                snprintf(filename, sizeof(filename), "%s/%d_%d_amplitude.txt", dir_path, num_qubits, depth);

                // define the output file name
                sprintf(filename, "Output/%d/%d_%d_amplitude.txt", loop, num_qubits, depth);
                ...
                sprintf(filename2, "/home/hai/FQsun/data/qrc/cpu/%d/%d_%d.txt", loop, num_qubits, depth);
                QRC(filename2);
