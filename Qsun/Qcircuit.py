...
def Qubit(qubit_num):
    ...
    amplitude_vector = np.zeros(2**qubit_num, dtype = np.complex64)
...
def Walk_Qubit(qubit_num=1, dim=1):
    ...
    if dim != 1 and dim != 2:
        ...
        if dim == 1:
            ...
            amplitude_vector = np.zeros(4*qubit_num-2, dtype = np.complex64)
            ...
            amplitude_vector = np.zeros(4*(2*qubit_num-1)**2, dtype = np.complex64)
