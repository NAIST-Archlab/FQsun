...
// Function to convert float to 16-bit fixed-point
int16_t floatToFixedPoint(float input) {
    // Define the scaling factor for the fractional part
    const int16_t scaleFactor = 1 << 14;  // 2^14
    ...
    int16_t fixedPointValue = (int16_t)(input * scaleFactor);
    
    return fixedPointValue;
}

float fixedPointToFloat(int16_t fixedPointValue) {
    // Define the scaling factor for the fractional part
    const float scaleFactor = 1.0f / (1 << 14);  // 1 / 2^14
...
void RX_Gate(int n, float phi){
    ...
    *(CGRA_info.reg_mmap + QCTR_N_CUT_MEM_BASE_PHYS + addr) = ((U32)n << 18) | cut;
    ...
    uint32_t sin_n_half = floatToFixedPoint(sin_phi);
    uint32_t cos_n_half = floatToFixedPoint(cos_phi);
...
void write_half_precision_to_file(FILE *file, uint64_t value) {
    ...
    float real = fixedPointToFloat(msb);
    float imag = fixedPointToFloat(lsb);
...
int main(){
	...
	for(int num_qubits = 3; num_qubits < 19; num_qubits++){
		...
		sprintf(filename, "Output/QFT_FX16_Output_%d_qubit.txt", num_qubits);
