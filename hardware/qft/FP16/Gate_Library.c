...
void RX_Gate(int n, float phi){
    ...
    *(CGRA_info.reg_mmap + QCTR_N_CUT_MEM_BASE_PHYS + addr) = ((U32)n << 18) | cut;
    ...
    uint16_t sin_n_half = float_to_half(sin_phi);
    uint16_t cos_n_half = float_to_half(cos_phi);
...
float half_to_float(uint16_t h) {
    uint16_t h_exp = (h >> 10) & 0x1F;  // Extract exponent
    uint16_t h_mant = h & 0x3FF;        // Extract mantissa
    uint16_t h_sign = h >> 15;          // Extract sign
...
void print_half_precision(U32 value) {
...
int main(){
    ...
    // for(int k = 0; k < qubit_num; k++)
		// H_Gate(qubit_num-k-1);
	// for(int k = 0; k < qubit_num; k++)
		// S_Gate(k);
	// for(int k = 0; k < qubit_num; k++)
		// RX_Gate(k, 0.5);
	// for(int k = 0; k < qubit_num; k++)
		// RZ_Gate(qubit_num-k-1, 0.2);
	// for(int k = 0; k < qubit_num; k++)
		// RY_Gate(k, 0.3);
	...
	while(num_qubits!=0){
		...
		for(int j = 0; j < num_qubits; j++){
			RX_Gate(num_qubits, 3.14159265359/pow(2, (num_qubits-j ))/ 2);
    ...
    dma_write(PING_MEM_BASE_PHYS, PING_MEM_BASE_PHYS, qubit);
    dma_write(PONG_MEM_BASE_PHYS, PONG_MEM_BASE_PHYS, qubit);
    
    Start();
    Wait();
	if((addr&0x1) == 0){
		dma_read(PING_MEM_BASE_PHYS, PING_MEM_BASE_PHYS, qubit);
		for (int k = 0; k < qubit; k++) {
        //printf("Pong_Mem[%d]: %08x\n", k, Pong_Mem[k]);
		print_half_precision(Ping_Mem[k]);
		}
	}
	else {
		dma_read(PONG_MEM_BASE_PHYS, PONG_MEM_BASE_PHYS, qubit);
		for (int k = 0; k < qubit; k++) {
        //printf("Pong_Mem[%d]: %08x\n", k, Pong_Mem[k]);
		print_half_precision(Pong_Mem[k]);
		}
	}
